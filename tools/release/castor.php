...
function release(): int
{
    ...
    $artefactsDir = __DIR__ . '/artefacts';
    fs()->remove($artefactsDir);
    fs()->mkdir($artefactsDir);

    check(
        'Download artifacts',
        'Failed to download artifacts.',
        fn () => run(['gh', 'run', 'download', $runArtifacts['databaseId'], '--dir', $artefactsDir], context: context()->withQuiet())->isSuccessful(),
    );
    ...
    check(
        'Check the number of artefacts',
        'There are not enough files in the artefacts directory.',
        fn () => EXPECTED_ARTIFACTS === count($files),
    );

    check(
        'Check if artefacts are not empty',
        'Some artefacts are empty.',
        fn () => !array_filter(
